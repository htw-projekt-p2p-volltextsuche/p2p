...
// src: https://github.com/libp2p/js-libp2p-kad-dht/blob/fd60ebfad06eae29d13156355338fabcb3c3a23a/src/routing.js

// @ts-ignore
const KBucket = require( "./modifiedKBucket" );
...
class RoutingTable {
  ...
  async _onInit( kBucketSize ) {
    ...
    this.kb.on( "removed", contact => { console.log( "kremove:", contact.peer.toB58String() ); } );
    this.kb.on( "added", contact => { console.log( "kadd:", contact.peer.toB58String() ); } );
  }
  ...
  closestPeers( key, count ) {
    /** @type {KBucketPeer[]} */
    const closest = this.kb.closest( key, 20 );

    const r = closest.map( p => p.peer );
    console.log( "r:", r.map( p => p.toB58String() ) );
    return r;
  }

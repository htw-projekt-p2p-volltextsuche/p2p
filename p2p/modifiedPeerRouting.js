...
// src: https://github.com/libp2p/js-libp2p-kad-dht/blob/fd60ebfad06eae29d13156355338fabcb3c3a23a/src/peer-routing/index.js
...
const c = require('../node_modules/libp2p-kad-dht/src/constants')
const Message = require('../node_modules/libp2p-kad-dht/src/message')
const Query = require('../node_modules/libp2p-kad-dht/src/query')
...
module.exports = (dht, self) => {
   ...
   */
    ...
    async * getClosestPeers (key, options = { shallow: false }) {
      const N_OF_RESULTS = 1;
      dht._log('getClosestPeers to %b', key)

      const id = await utils.convertBuffer(key)
      const closestPeers = dht.routingTable.closestPeers(id, dht.kBucketSize)

      return yield closestPeers[0];

      const tablePeers = closestPeers.filter( p => p.toB58String() !== self.toB58String() )
      ...
      for (const pId of sorted.slice(0, N_OF_RESULTS)) {
        yield pId
      }
    },
